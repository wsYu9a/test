...
public final class SafeCollector<T> extends ContinuationImpl implements FlowCollector<T>, CoroutineStackFrame {
    ...
    public SafeCollector(@k FlowCollector<? super T> flowCollector, @k CoroutineContext coroutineContext) {
    ...
    public Object invokeSuspend(@k Object r42) {
        Throwable m61exceptionOrNullimpl = Result.m61exceptionOrNullimpl(r42);
        if (m61exceptionOrNullimpl != null) {
            this.lastEmissionContext = new DownstreamExceptionContext(m61exceptionOrNullimpl, getContext());
        }
        Continuation<? super Unit> continuation = this.completion;
        if (continuation != null) {
            continuation.resumeWith(r42);
        }
