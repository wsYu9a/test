...
public class ParserConfig {
    ...
    public Class<?> checkAutoType(String str, Class<?> cls, int i10) {
        if (str == null) {
            return null;
        }
        if (str.length() < 128) {
            if (str.length() >= 3) {
                ...
                int i12 = Feature.SupportAutoType.mask;
                if ((i10 & i12) != 0 || (i12 & JSON.DEFAULT_PARSER_FEATURE) != 0 || this.autoTypeSupport) {
                    TypeUtils.addMapping(str, loadClass);
                    return loadClass;
                }
                throw new JSONException("autoType is not support : " + str);
