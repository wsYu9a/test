package qe;

import android.graphics.Bitmap;

/* loaded from: classes3.dex */
public final class e extends t5.e {

    /* renamed from: c */
    public final byte[] f30041c;

    /* renamed from: d */
    public final int f30042d;

    /* renamed from: e */
    public final int f30043e;

    /* renamed from: f */
    public final int f30044f;

    /* renamed from: g */
    public final int f30045g;

    public e(byte[] bArr, int i10, int i11, int i12, int i13, int i14, int i15) {
        super(i14, i15);
        if (i14 + i12 > i10 || i15 + i13 > i11) {
            throw new IllegalArgumentException("Crop rectangle does not fit within image data.");
        }
        this.f30041c = bArr;
        this.f30042d = i10;
        this.f30043e = i11;
        this.f30044f = i12;
        this.f30045g = i13;
    }

    @Override // t5.e
    public byte[] c() {
        int e10 = e();
        int b10 = b();
        int i10 = this.f30042d;
        if (e10 == i10 && b10 == this.f30043e) {
            return this.f30041c;
        }
        int i11 = e10 * b10;
        byte[] bArr = new byte[i11];
        int i12 = (this.f30045g * i10) + this.f30044f;
        if (e10 == i10) {
            System.arraycopy(this.f30041c, i12, bArr, 0, i11);
            return bArr;
        }
        for (int i13 = 0; i13 < b10; i13++) {
            System.arraycopy(this.f30041c, i12, bArr, i13 * e10, e10);
            i12 += this.f30042d;
        }
        return bArr;
    }

    @Override // t5.e
    public byte[] d(int i10, byte[] bArr) {
        if (i10 < 0 || i10 >= b()) {
            throw new IllegalArgumentException("Requested row is outside the image: " + i10);
        }
        int e10 = e();
        if (bArr == null || bArr.length < e10) {
            bArr = new byte[e10];
        }
        System.arraycopy(this.f30041c, ((i10 + this.f30045g) * this.f30042d) + this.f30044f, bArr, 0, e10);
        return bArr;
    }

    @Override // t5.e
    public boolean g() {
        return true;
    }

    public int k() {
        return this.f30043e;
    }

    public int l() {
        return this.f30042d;
    }

    public Bitmap m() {
        int e10 = e();
        int b10 = b();
        int[] iArr = new int[e10 * b10];
        int i10 = (this.f30045g * this.f30042d) + this.f30044f;
        for (int i11 = 0; i11 < b10; i11++) {
            int i12 = i11 * e10;
            for (int i13 = 0; i13 < e10; i13++) {
                iArr[i12 + i13] = ((this.f30041c[i10 + i13] & 255) * 65793) | (-16777216);
            }
            i10 += this.f30042d;
        }
        Bitmap createBitmap = Bitmap.createBitmap(e10, b10, Bitmap.Config.ARGB_8888);
        createBitmap.setPixels(iArr, 0, e10, 0, 0, e10, b10);
        return createBitmap;
    }
}
