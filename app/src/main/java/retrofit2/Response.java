...
public final class Response<T> {
    ...
    public static <T> Response<T> error(int i10, ResponseBody responseBody) {
        ...
        if (i10 >= 400) {
            return error(responseBody, new Response.Builder().body(new OkHttpCall.NoContentResponseBody(responseBody.get$contentType(), responseBody.getContentLength())).code(i10).message("Response.error()").protocol(Protocol.HTTP_1_1).request(new Request.Builder().url("http://localhost/").build()).build());
        }
        throw new IllegalArgumentException("code < 400: " + i10);
    }

    public static <T> Response<T> success(@Nullable T t10) {
        return success(t10, new Response.Builder().code(200).message(bz.f6993k).protocol(Protocol.HTTP_1_1).request(new Request.Builder().url("http://localhost/").build()).build());
    }
    ...
    public static <T> Response<T> success(@Nullable T t10, Headers headers) {
        ...
        return success(t10, new Response.Builder().code(200).message(bz.f6993k).protocol(Protocol.HTTP_1_1).headers(headers).request(new Request.Builder().url("http://localhost/").build()).build());
    }
