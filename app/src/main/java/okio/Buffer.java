...
public final class buffer implements BufferedSource {
    ...
    @Override // okio.BufferedSource
    public long indexOfElement(@k ByteString targetBytes) {
    ...
    public InputStream inputStream() {
        return new InputStream() { // from class: okio.RealBufferedSource$inputStream$1
            public RealBufferedSource$inputStream$1() {
            }
            ...
            @Override // java.io.InputStream
            public int read(@k byte[] r82, int r92, int byteCount) {
                Intrinsics.checkNotNullParameter(r82, "data");
                if (!buffer.this.closed) {
                    _UtilKt.checkOffsetAndCount(r82.length, r92, byteCount);
                    ...
                    return buffer.this.bufferField.read(r82, r92, byteCount);
                }
                throw new IOException("closed");
            }
        };
    ...
    @Override // okio.BufferedSource
    public boolean rangeEquals(long r82, @k ByteString bytes) {
        Intrinsics.checkNotNullParameter(bytes, "bytes");
        return rangeEquals(r82, bytes, 0, bytes.size());
    }

    @Override // okio.BufferedSource
    public int read(@k byte[] sink) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        return read(sink, 0, sink.length);
    }

    @Override // okio.BufferedSource
    public long readAll(@k Sink sink) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        long j10 = 0;
    ...
    */
    ...
    @Override // okio.BufferedSource
    public void readFully(@k byte[] sink) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        try {
            require(sink.length);
    ...
    @Override // okio.BufferedSource
    @k
    public String readString(long byteCount, @k Charset r42) {
        Intrinsics.checkNotNullParameter(r42, "charset");
        require(byteCount);
        return this.bufferField.readString(byteCount, r42);
    }
    ...
    @Override // okio.BufferedSource
    public int select(@k Options options) {
    ...
    @Override // okio.BufferedSource
    public long indexOfElement(@k ByteString targetBytes, long fromIndex) {
        Intrinsics.checkNotNullParameter(targetBytes, "targetBytes");
        if (!(!this.closed)) {
            throw new IllegalStateException("closed".toString());
        }
    ...
    @Override // okio.BufferedSource
    public long indexOf(@k ByteString bytes) {
        Intrinsics.checkNotNullParameter(bytes, "bytes");
        return indexOf(bytes, 0L);
    }

    @Override // okio.BufferedSource
    public boolean rangeEquals(long r82, @k ByteString bytes, int bytesOffset, int byteCount) {
        int i10;
        Intrinsics.checkNotNullParameter(bytes, "bytes");
        if (!this.closed) {
            if (r82 >= 0 && bytesOffset >= 0 && byteCount >= 0 && bytes.size() - bytesOffset >= byteCount) {
                while (i10 < byteCount) {
                    long j10 = i10 + r82;
                    i10 = (request(1 + j10) && this.bufferField.getByte(j10) == bytes.getByte(bytesOffset + i10)) ? i10 + 1 : 0;
                }
                return true;
            }
            return false;
    ...
    @Override // okio.BufferedSource
    @k
    public String readString(@k Charset r32) {
        Intrinsics.checkNotNullParameter(r32, "charset");
        this.bufferField.writeAll(this.source);
        return this.bufferField.readString(r32);
    }
    ...
    @Override // okio.BufferedSource
    public void readFully(@k Buffer sink, long byteCount) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        try {
            require(byteCount);
    ...
    @Override // okio.BufferedSource
    public int read(@k byte[] sink, int r11, int byteCount) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        long j10 = byteCount;
        _UtilKt.checkOffsetAndCount(sink.length, r11, j10);
        if (this.bufferField.size() == 0 && this.source.read(this.bufferField, PlaybackStateCompat.ACTION_PLAY_FROM_URI) == -1) {
            return -1;
        }
        return this.bufferField.read(sink, r11, (int) Math.min(j10, this.bufferField.size()));
    }

    @Override // okio.BufferedSource
    public long indexOf(@k ByteString bytes, long fromIndex) {
        Intrinsics.checkNotNullParameter(bytes, "bytes");
        if (!(!this.closed)) {
            throw new IllegalStateException("closed".toString());
        }
    ...
    public int read(@k ByteBuffer sink) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        if (this.bufferField.size() == 0 && this.source.read(this.bufferField, PlaybackStateCompat.ACTION_PLAY_FROM_URI) == -1) {
            return -1;
        }
